...
def get_crm_form_script():
	...
	setActions() {
		...
		this.actions.push({
			...
			onClick: () => {
				call(
					"antmed_crm.fcrm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_quotation_url",
					{
		...
		call("antmed_crm.fcrm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_customer_link", {
			crm_deal: this.doc.name
		}).then((customer_url) => {
			if (customer_url) {
				this.actions.push({
