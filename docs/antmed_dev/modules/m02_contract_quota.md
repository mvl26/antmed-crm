...
| FE pages | `frontend/src/pages/Antmed*` + route `/antmed/contracts`, `/antmed/contracts/:name`, `/antmed/contract-health` |
...
| Site dev | `miyano` |
| Trạng thái docs | DESIGN — Slice **M02-1 FROZEN** (spec-contract chốt, BE/FE code được); M02-2/3/4 đề xuất |
| Cập nhật | 2026-06-17 |
...
> - **Always**: tạo đúng 2 DocType (`AntMed Contract` submittable + `AntMed Quota Item` child) + đúng 2 endpoint đọc (`list_contracts`/`get_contract`) + DocPerm VI theo ma trận §1bis.4 + FE list/detail render **read-only** + naming sinh `AM-HD-2026-00001`.
> - **Never**: KHÔNG fixture `workflow.json`; KHÔNG enforce BR-01/02/06; KHÔNG `AntMed Quota Usage Log`/`AntMed Contract Amendment`; KHÔNG endpoint `get_contract_health`/`list_quota_alerts`/`check_item_in_contract`; KHÔNG route `/antmed/contract-health`; KHÔNG `doc_events` mới trong `crm/hooks.py`; KHÔNG Link→`AntMed Item` (M03 chưa land → dùng `Data`); KHÔNG `createListResource` (list trả dict bọc); KHÔNG axios / `antmed_crm.api.*`; KHÔNG Role `AM System Admin`.
> - **Ask-first**: muốn thêm Workflow/usage-log/enforce → đó là Slice M02-2/3 (mở vòng factory mới), KHÔNG nhét vào M02-1.
...
### 1bis.5 — FE (FROZEN — read-only)

| Route (APPEND lazy vào `frontend/src/router.js`) | name | page |
|---|---|---|
| `/antmed/contracts` | `AntmedContracts` | `frontend/src/pages/AntmedContracts.vue` |
| `/antmed/contracts/:name` | `AntmedContractDetail` | `frontend/src/pages/AntmedContractDetail.vue` |

- **List**: `createResource` → `crm.api.antmed.contract.list_contracts`; đọc **`r.data.data`** (list trả dict bọc — pattern verified R2, KHÔNG `createListResource`). Cột: Số HĐ / Bệnh viện (hospital_name) / Hiệu lực đến / Giá trị / badge `status`. Lọc theo BV + status, search số HĐ. Click dòng → `router.push('/antmed/contracts/' + name)`.
- **Detail**: `createResource` → `crm.api.antmed.contract.get_contract`; render **read-only**: header (contract_no, hospital_name, badge status, ngày ký/hiệu lực, giá trị) + bảng danh mục quota (mỗi dòng: item/item_name/uom/đơn giá/quota_qty/% còn lại `remaining_pct`/cờ lock).
- **Nhãn 100% tiếng Việt qua `__()`**: badge status (Nháp/Hiệu lực/Sắp hết hạn/Hết hạn/Đã huỷ) + header cột + nhãn "Còn lại %". KHÔNG hardcode chuỗi EN.
- **KHÔNG** route `/antmed/contract-health` ở M02-1 (đó là M02-2). Sidebar `antmedNav.js`: mục "Hợp đồng" chuyển từ stub "Sắp có" → enabled trỏ `/antmed/contracts`.
...
| Route (THÊM, lazy) | name | page | Mô tả | Role dùng |
|---|---|---|---|---|
| `/antmed/contracts` | `AntmedContracts` | `pages/AntmedContracts.vue` | List HĐ: cột Số HĐ, BV, hiệu lực đến, giá trị, trạng thái; lọc BV/trạng thái; search | `Quản lý`, `NV kinh doanh` |
| `/antmed/contracts/:name` | `AntmedContractDetail` | `pages/AntmedContractDetail.vue` | Chi tiết HĐ: header + danh mục SKU/đơn giá/quota (progress bar/item), timeline ký→hiệu lực, đính kèm | `Quản lý`, `NV kinh doanh` |
| `/antmed/contract-health` | `AntmedContractHealth` | `pages/AntmedContractHealth.vue` | "Sức khoẻ hợp đồng": list HĐ + progress bar 2 màu (xanh ≤80% / cam 80–100% / đỏ >100% hoặc <30% còn ≤30 ngày) | `Quản lý`, CEO |

- **List** (`/antmed/contracts`): `createResource` → `crm.api.antmed.contract.list_contracts`; click dòng → `router.push` detail.
...
1. **Slice M02-1 — Contract master + quota (đọc) ✅ spec FROZEN §1bis**: DocType `AntMed Contract` (submittable, `track_changes=1`, naming `AM-HD-.YYYY.-.#####`, Select `status` read-only — KHÔNG Workflow) + `AntMed Quota Item` child (`item`=Data tạm) + DocPerm VI (NV read-only); API `list_contracts`/`get_contract` (RAW dict, count==rows); FE 2 route list+detail read-only; test BE (tồn tại + naming AM-HD + unique contract_no + count==rows + submit→docstatus 1 + permission 403), FE vitest + build. *(KHÔNG enforce BR; KHÔNG workflow.json; KHÔNG usage-log; KHÔNG contract-health.)*
...
**FE (vitest + build) — Slice M02-1:** `frontend/tests/unit/antmedContract.test.js` — **2 route** mới tồn tại (`/antmed/contracts` name `AntmedContracts` + `/antmed/contracts/:name` name `AntmedContractDetail`, lazy); page list gọi `crm.api.antmed.contract.list_contracts` đọc `r.data.data`; detail gọi `crm.api.antmed.contract.get_contract`; KHÔNG `antmed_crm.api`/axios/tanstack/`createListResource`. `yarn build` emit chunk Antmed* không vỡ + `crm.html` regenerate. *(route `/antmed/contract-health` + `get_contract_health` = M02-2, KHÔNG test ở M02-1.)*

**Pixel (Playwright, sau USER reload) — Slice M02-1:** `http://miyano:8000/crm/antmed/contracts` render list HĐ thật → filter BV/status + search số HĐ → click dòng → `/antmed/contracts/:name` detail (header + bảng danh mục quota, badge status VI, read-only) → quay lại; 0 console error; network API 200; verify count==rows trên UI + permission 403 khi user thiếu read. *(contract-health = M02-2.)*
