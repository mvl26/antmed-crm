...
- **In-place** trong app `crm`: BE ở `crm/antmed/` + `crm/api/antmed/` → endpoint `crm.api.antmed.<module>.<fn>`. FE ở `frontend/src` (page `Antmed<Feature>.vue`, route `/antmed/*`). Site = `miyano`.
