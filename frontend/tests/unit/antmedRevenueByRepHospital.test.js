...
describe('M02-10 data layer — getRevenueByRepHospital url + method GET', () => {
  ...
  it('url == antmed_crm.api.antmed.sales_team.revenue_by_rep_hospital (KHÔNG prefix sai)', () => {
    expect(dataSrc).toMatch(
      /antmed_crm\.api\.antmed\.sales_team\.revenue_by_rep_hospital/,
    )
    expect(dataSrc).not.toMatch(
      /['"]crm\.api\.antmed\.sales_team\.revenue_by_rep_hospital/,
    )
  })

  it("resource có method === 'GET' (chống defect POST→403)", () => {
    const block = dataSrc.slice(
      dataSrc.indexOf('export function getRevenueByRepHospital'),
    )
...
describe('M02-10 heatCellClass — map 4 mức heat (PURE, KHÔNG hex)', () => {
  it('HEAT_CELL_COLORS đủ 4 KEY khớp BE (h2c/h3c/h4c/h5c)', () => {
    expect(Object.keys(HEAT_CELL_COLORS).sort()).toEqual([
      'h2c',
      'h3c',
      'h4c',
      'h5c',
...
describe('M02-10 legend 4 mức heat (HEAT_LEGEND SSoT)', () => {
  it('HEAT_LEGEND đúng 4 mục thứ tự Thấp→TB→Cao→Rất cao', () => {
    expect(HEAT_LEGEND.map((l) => l.label)).toEqual([
      'Thấp',
      'TB',
      'Cao',
      'Rất cao',
...
describe('M02-10 SSR render-verify — card NV×BV render HTML thật (heat + — + Tổng đậm)', () => {
  async function renderTemplate(
    ...
    const comp = {
      ...
      setup() {
        ...
        const revenue = {
          data: { months: [], groups: [], grand_total: 1 },
          loading: false,
          error: null,
          reload() {},
        }
    ...
    rows: [
      {
        ...
        cells: [
          {
            hospital: 'BM',
            hospital_label: 'Bạch Mai',
            value: 2_140_000_000,
            heat: 'h5c',
          },
          {
            hospital: 'VD',
            hospital_label: 'Việt Đức',
            value: 680_000_000,
            heat: 'h3c',
          },
          null,
  ...
  it('render data: tiêu đề + cột BV + Tổng (tr) + hàng full_name + heat class + cell rỗng —', async () => {
    ...
    // legend 4 mức.
    for (const lbl of ['Thấp', 'TB', 'Cao', 'Rất cao'])
