...
async function addNoteToCallLog(_note, isInsert = false) {
  if (isInsert && _note.name) {
    await call('antmed_crm.integrations.api.add_note_to_call_log', {
...
async function addTaskToCallLog(_task, isInsert = false) {
  if (isInsert && _task.name) {
    await call('antmed_crm.integrations.api.add_task_to_call_log', {
      call_sid: callLog.value?.data?.id,
      task: _task,
    })
...
async function createLead() {
  ...
  call('antmed_crm.fcrm.doctype.crm_call_log.crm_call_log.create_lead_from_call_log', {
